...
export default function TelegramSendModal({ 
  ...
  const buildHeartbeatMessage = useCallback((statsData, coinSymbol, isForce = false) => {
...
🔗 <b>Coin:</b> ${coinSymbol}
  ...
  const sendHeartbeat = useCallback(async (isForce = false) => {
    if (!stats) {
    ...
    setIsSending(true);
    setSendStatus(isForce ? '🚀 Sending force heartbeat...' : '🔄 Sending heartbeat...');
    ...
    try {
      ...
      if (!result?.ok) {
        throw new Error(result?.error || 'Failed to send heartbeat notification.');
      }
...
• Total: <b>${summary.total || 0}</b>
      ...
      setSendStatus(isForce ? '✅ Force heartbeat sent successfully!' : '✅ Heartbeat sent successfully!');
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="📨 Telegram Mining Notifications" maxWidth="550px">
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', padding: '8px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: isHeartbeatRunning ? 'rgba(248,113,113,0.2)' : 'rgba(52,211,153,0.2)',
        ...
        <div style={{ 
          ...
          <button
            ...
            style={{
              ...
              fontSize: '12px',
              border: 'none',
              background: (isForceRunning || isSending || loading || !stats) ? 'rgba(148,163,184,0.1)' : 'rgba(251,191,36,0.2)',
              color: (isForceRunning || isSending || loading || !stats) ? '#64748b' : '#fbbf24',
              cursor: (isForceRunning || isSending || loading || !stats) ? 'default' : 'pointer',
              opacity: (isForceRunning || isSending || loading || !stats) ? 0.5 : 1,
              fontWeight: 600,
            }}
          >
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px' }}>
          <span style={{ fontSize: '12px', color: sendStatus.includes('✅') ? '#34d399' : sendStatus.includes('❌') ? '#f87171' : '#94a3b8' }}>
            {sendStatus || 'Ready'}
          </span>
          <div style={{ display: 'flex', gap: '8px' }}>
            <button
              ...
              style={{
                ...
                fontSize: '12px',
                border: '1px solid rgba(148,163,184,0.2)',
                background: (isSending || !stats || loading) ? 'rgba(148,163,184,0.05)' : 'rgba(148,163,184,0.1)',
                color: '#e2e8f0',
            ...
            <button
              ...
              style={{
                ...
                fontSize: '12px',
                border: 'none',
                background: (message.trim() && !isSending && !loading) ? 'rgba(52,211,153,0.2)' : 'rgba(148,163,184,0.1)',
                color: (message.trim() && !isSending && !loading) ? '#34d399' : '#64748b',
                cursor: (isSending || !message.trim() || loading) ? 'default' : 'pointer',
        ...
        {stats && (
          <div style={{ 
            ...
            <div>⛏️ Hashrate: {stats.liveStats?.currentHashrate || 'N/A'} | 15m: {stats.liveStats?.avg15m || 'N/A'}</div>
            <div>💰 Pending: {stats.paymentStats?.pendingBalance || 'N/A'} | Workers: {stats.liveStats?.workersOnline || 0}/{stats.liveStats?.workersTotal || 0}</div>
            <div>📈 Efficiency: {stats.shareStats?.total?.efficiency || 'N/A'}% | Blocks: {stats.blockStats?.totalBlocks || 0}</div>
            {isHeartbeatRunning && (
              <div style={{ color: '#34d399' }}>🔄 Heartbeat active - next send in ~15m</div>
            )}
          </div>
        )}
      </div>
