  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '10px', color: '#94a3b8' }}>
      ...
      {lastUpdated && !isLoading && (
        <span style={{ color: '#64748b', marginLeft: '4px' }}>({new Date(lastUpdated).toLocaleTimeString()})</span>
      )}
    </div>
  );
}
  ...
  const handleUpdatePrices = useCallback(async () => {
    await triggerPriceUpdate(async () => {
      try {
        ...
        refresh(true); // Refresh data after successful price update
      } catch (err) { console.error("[Prices] Update failed:", err.message); throw err; }
    });
  }, [onCall, triggerPriceUpdate]);
  ...
  return (
    <section style={{ display: "grid", gap: "10px", marginBottom: "12px" }}>
      ...
      <div style={{
        ...
        border: "1px solid rgba(148,163,184,0.10)", background: "rgba(2,6,23,0.45)",
      }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <button className="btn-pro secondary" onClick={handleUpdatePrices} disabled={priceUpdateStatus === "running"} style={{ fontSize: "11px", padding: "4px 12px" }}>
            {priceUpdateButtonText}
          </button>
          <button className="btn-pro secondary" onClick={() => void refresh(true)} disabled={loading} style={{ fontSize: "11px", padding: "4px 12px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.1fr 0.9fr", gap: "10px" }}>
        <div style={{
          padding: "12px", borderRadius: "12px", border: "1px solid rgba(148,163,184,0.12)",
          ...
          <div style={{ display: "flex", justifyContent: "space-between", gap: "10px", flexWrap: "wrap", marginBottom: "8px" }}>
            <div>
              ...
              <div style={{ color: "#f8fafc", fontSize: "15px", fontWeight: 800 }}>Best route</div>
            </div>
          </div>
          {error && <div style={{ color: "#f87171", fontSize: "12px", marginBottom: "10px" }}>{error}</div>}
          {bestRoute ? (
            <div style={{
              display: "grid", gap: "4px", padding: "10px", borderRadius: "10px",
              background: "rgba(2,6,23,0.45)", border: "1px solid rgba(148,163,184,0.10)",
            }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: "10px", flexWrap: "wrap" }}>
                <div>
                  <div style={{ color: "#e2e8f0", fontSize: "18px", fontWeight: 800 }}>{bestOpportunity.label}</div>
                  <div style={{ color: "#94a3b8", fontSize: "10px" }}>{bestOpportunity.bestSource} to NiceHash / MRR mapping</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ color: "#34d399", fontSize: "18px", fontWeight: 800 }}>{btcValue(bestOpportunity.miningDutchBtcPerDay)}</div>
            ...
            <div style={{ color: "#94a3b8", fontSize: "12px", padding: "8px 0" }}>No route data yet. Refresh once the pool stats finish loading.</div>
          )}
        ...
        <div style={{
          ...
          <div style={{ color: "#f8fafc", fontSize: "13px", fontWeight: 800, marginBottom: "8px" }}>Top Route Candidates</div>
          <div style={{ display: "grid", gap: "8px", maxHeight: "170px", overflow: "auto" }}>
            ...
            {opportunities.length === 0 && <div style={{ color: "#94a3b8", fontSize: "12px" }}>No route candidates available.</div>}
          </div>
        </div>
      </div>
      ...
      <div style={{
        padding: "12px", borderRadius: "12px", border: "1px solid rgba(148,163,184,0.12)",
        background: "rgba(15,23,42,0.72)", boxShadow: "0 18px 40px rgba(0,0,0,0.20)",
      }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: "10px", flexWrap: "wrap", marginBottom: "10px" }}>
          ...
          <div style={{ color: "#94a3b8", fontSize: "12px" }}>Compare the same algo/day across all three sources</div>
        </div>

        {bestOpportunity ? (
          <div style={{
            display: "grid", gap: "6px", marginBottom: "10px", padding: "10px", borderRadius: "10px",
            ...
            <div style={{ display: "flex", justifyContent: "space-between", gap: "12px", flexWrap: "wrap" }}>
              <div>
                ...
                <div style={{ color: "#94a3b8", fontSize: "10px" }}>Winner: {bestOpportunity.winner} · NiceHash {bestOpportunity.nicehashAlgo} · MRR {bestOpportunity.mrrAlgo}</div>
              </div>
              <div style={{ textAlign: "right" }}>
                <div style={{ color: bestOpportunity.opportunityScore >= 0 ? "#34d399" : "#f87171", fontSize: "15px", fontWeight: 900 }}>{btcValue(bestOpportunity.opportunityScore)}</div>
        ...
        {priceFetchStatus && Object.keys(priceFetchStatus).length > 0 && (
          <div style={{ marginTop: "12px", padding: "10px", borderRadius: "8px", background: "rgba(15,23,42,0.5)", border: "1px solid rgba(148,163,184,0.12)" }}>
            <details>
              <summary style={{ color: "#94a3b8", fontSize: "11px", cursor: "pointer" }}>Price Fetch Status ({Object.keys(priceFetchStatus).length} algos)</summary>
                    ...
                    <div style={{ display: "flex", gap: "4px", flexWrap: "wrap" }}>
                      {row.heroCoins.map((coin) => (
                        <button key={coin} onClick={() => openCoinModal(coin)} style={{
                          ...
                          padding: "2px 6px", fontSize: "10px", cursor: "pointer", transition: "all 0.2s",
                        }}
                          onMouseEnter={(e) => { e.target.style.background = "rgba(37,99,235,0.25)"; e.target.style.borderColor = "rgba(96,165,250,0.5)"; }}
                          onMouseLeave={(e) => { e.target.style.background = "rgba(37,99,235,0.12)"; e.target.style.borderColor = "rgba(96,165,250,0.22)"; }}
                        >{coin}</button>
                      ))}
                    </div>
                  </BodyCell>
  ...
  return (
    <TelegramMineProvider onCall={onCall}>
      <div className="app-shell mining-shell" style={{
        padding: "0 12px 24px", maxWidth: "1500px", margin: "0 auto",
        ...
        <header style={{
          padding: "20px 0 12px", marginBottom: "12px", borderBottom: "1px solid rgba(148,163,184,0.10)",
          display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: "16px", flexWrap: "wrap",
        }}>
          ...
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            <button className="btn-pro secondary" onClick={onNavigateHome}>Back to Dashboard</button>
            <button className="btn-pro secondary" onClick={() => setTelegramModalOpen(true)}>Telegram</button>
            <button className="btn-pro secondary" onClick={() => window.location.reload()}>Refresh Page</button>
          </div>
        </header>
        ...
        <section style={{ display: "grid", gridTemplateColumns: "1.15fr 0.85fr", gap: "12px", alignItems: "start" }}>
          <article style={{ padding: "12px", background: "rgba(15,23,42,0.72)", border: "1px solid rgba(148,163,184,0.12)", borderRadius: "12px", boxShadow: "0 18px 40px rgba(0,0,0,0.20)" }}>
            <HeroMinersCard onCall={onCall} />
          </article>
          <aside style={{ padding: "12px", background: "rgba(15,23,42,0.72)", border: "1px solid rgba(148,163,184,0.12)", borderRadius: "12px", boxShadow: "0 18px 40px rgba(0,0,0,0.20)" }}>
