...
function MiningRouteHero({ onCall }) {
  ...
  const handleForceHeartbeat = useCallback(async () => {
    try {
      await sendMineNotice("🚀 Manual heartbeat triggered for rental monitor...");
      const res = await onCall("/api/v2/mrr/monitor/run", {
        method: "POST",
        body: { client: "ALL" },
        silent: true,
      });
      await sendMineNotice(`✅ Heartbeat complete. Rented: ${res?.summary?.totals?.rented || 0}, Ghosts: ${res?.summary?.totals?.ghost || 0}`);
    } catch (err) {
      await sendMineNotice(`❌ Heartbeat failed: ${err.message}`);
    }
  }, [onCall, sendMineNotice]);
