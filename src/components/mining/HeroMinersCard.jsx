    ...
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "11px" }}>
      <thead>
        <tr style={{ color: "#64748b", borderBottom: "1px solid #334155" }}>
          <th style={{ padding: "6px 4px", textAlign: "left", cursor: "pointer" }} onClick={() => onSort("algorithm")}>
            Algorithm
          </th>
          <th style={{ padding: "6px 4px", textAlign: "right", cursor: "pointer" }} onClick={() => onSort("miners")}>
            Miners
          </th>
          <th style={{ padding: "6px 4px", textAlign: "right", cursor: "pointer" }} onClick={() => onSort("usdPerDay")}>
            USD/Day
          </th>
          <th style={{ padding: "6px 4px", textAlign: "right", cursor: "pointer" }} onClick={() => onSort("btcPerDay")}>
            BTC/Day
          </th>
        </tr>
      </thead>
      <tbody>
        {rows.map((item, idx) => (
          <tr key={`${item.normalized}-${idx}`} style={{ borderBottom: "1px solid #1e293b" }}>
      ...
      <div className="stats-section">
        ...
        <div className="stats-grid">
          <div className="stat-item">
            ...
            <span className="stat-value highlight">{liveStats.currentHashrate || "0 H/s"}</span>
          </div>
          <div className="stat-item">
          ...
          <div className="stat-item">
            ...
            <span className="stat-value">{liveStats.workersOnline || 0} / {liveStats.workersTotal || 0}</span>
          </div>
        </div>
      </div>
      ...
      <div className="stats-section">
        ...
        <div className="stats-grid">
          <div className="stat-item">
            ...
            <span className="stat-value success">{validShares.toLocaleString()}</span>
          </div>
          <div className="stat-item">
            <span className="stat-label">Stale Shares</span>
            <span className="stat-value warning">{staleShares.toLocaleString()}</span>
          </div>
          <div className="stat-item">
            <span className="stat-label">Invalid Shares</span>
            <span className="stat-value error">{invalidShares.toLocaleString()}</span>
          </div>
          <div className="stat-item">
        ...
        <div className="stats-section">
          ...
          <div className="stats-grid">
            ...
            <div className="stat-item">
              ...
              <span className="stat-value">{workerStats.totalHashrate || "0 H/s"}</span>
            </div>
          </div>
        </div>
      )}
...
export default function HeroMinersCard({ 
  ...
  const fetchCoinPrice = useCallback(async (coinSymbol) => {
    ...
    try {
      ...
      if (result?.success) {
        ...
        localStorage.setItem(
          ...
          JSON.stringify({ address: address.trim(), coin: coin.trim().toUpperCase() })
        );
  ...
  // ============================================
  return (
    <div className="herominers-card" style={{
      padding: "15px",
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
          {addressStats && (
            <button className="btn-sm" onClick={() => setViewMode(viewMode === "dashboard" ? "raw" : "dashboard")}>
          ...
          <label style={{ fontSize: "11px", display: "flex", alignItems: "center", gap: "6px", cursor: "pointer", color: "#94a3b8" }}>
            ...
            Mining Only
            {filterMiningOnly && <span style={{ fontSize: "10px", color: "#38bdf8" }}>({activeAlgos.size} active)</span>}
          </label>
          <button className="btn-pro secondary" onClick={refreshData} disabled={loading} style={{ fontSize: "11px", background: "transparent", border: "none", color: "#94a3b8", cursor: "pointer", textDecoration: "underline" }}>
            {loading ? "…" : "Refresh"}
          </button>
        </div>
      </div>
      ...
      <div className="search-section" style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
        <input
          type="text"
          value={coin}
          onChange={(e) => setCoin(e.target.value.toUpperCase())}
          placeholder="Coin (e.g., QRL)"
          style={{ flex: "1", minWidth: "80px", padding: "6px 10px", background: "rgba(255,255,255,0.05)", border: "1px solid #334155", borderRadius: "6px", color: "#e2e8f0", fontSize: "12px" }}
        />
        <input
          type="text"
          value={address}
          ...
          style={{ flex: "2", minWidth: "200px", padding: "6px 10px", background: "rgba(255,255,255,0.05)", border: "1px solid #334155", borderRadius: "6px", color: "#e2e8f0", fontSize: "12px" }}
        />
        {onPaste && addressStats?.coinStats && (
          <button className="btn-primary" onClick={() => onPaste(addressStats.coinStats)} style={{ padding: "6px 12px" }}>
            📋 Paste
          </button>
        )}
        <button className="btn-primary" onClick={handleLookup} disabled={addressLoading} style={{ padding: "6px 12px" }}>
          {addressLoading ? "⏳" : "🔍 Lookup"}
        </button>
      </div>

      {addressError && <div style={{ color: "#f87171", padding: "4px 10px", fontSize: "12px" }}>❌ {addressError}</div>}
      ...
      <div className="code-block-content" style={{
        ...
        {error && (
          <div style={{ color: "#f87171", padding: "10px" }}>
            <div>{error}</div>
            <button onClick={refreshData} style={{ marginTop: "8px", background: "rgba(255,255,255,0.1)", border: "1px solid #475569", color: "#e2e8f0", padding: "4px 12px", borderRadius: "4px", cursor: "pointer" }}>
              Retry
            </button>
          </div>
        )}
        {!error && !loading && (
          <HeroMinersTable stats={heroStats} filterMiningOnly={filterMiningOnly} activeAlgos={activeAlgos} sortConfig={sortConfig} onSort={requestSort} />
        )}
      </div>
      ...
      {addressStats && (
        <div style={{ marginTop: "12px", borderTop: "1px solid #1e293b", paddingTop: "12px" }}>
          {viewMode === "dashboard" ? (
            ...
            <pre className="raw-data" style={{ maxHeight: "400px", overflow: "auto", fontSize: "11px", background: "rgba(0,0,0,0.3)", padding: "12px", borderRadius: "8px" }}>
