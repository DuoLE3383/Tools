  ...
  const fetchAllData = useCallback(async (force = false) => {
    ...
    try {
      ...
      const [statsResult, profitResult] = await Promise.allSettled([
        (async () => {
          const stats = await fetchMiningStats("herominers", "ALL", null, null, 20000, force);
          return stats;
        })(),
        (async () => {
  ...
  return (
    <div style={{ padding: "12px", background: "#1e1e2e", border: "3px solid #000", boxShadow: "6px 6px 0px #000", display: "flex", flexDirection: "column", gap: "8px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
        <div>
          <h4 style={{ margin: 0, color: "#38bdf8", fontSize: "10px", fontWeight: 900, textTransform: "uppercase" }}>
            🔍 HEROMINERS PROFIT
          </h4>
