...
// REMOVED: import { fetchAndSaveCoinPrices, getCoinPricesFromDb } from './coinGecko/coinGeckoClient.js';
// REMOVED: import { getSimplePrice } from '../src/core/coinGecko.js';
...
// In-memory CoinGecko cache (replaces the duplicate Map)
let coinGeckoCache = {
  data: null,
  timestamp: 0,
...
async function getCachedCoinPrices(ids) {
  ...
  if (
    coinGeckoCache.data &&
    now - coinGeckoCache.timestamp < coinGeckoCache.ttl
  ) {
  ...
  try {
    ...
    return buildFallbackPrices(ids);
  }
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      res.json({
        ...
        data,
        source: "cache",
        cached: coinGeckoCache.data !== null,
      });
    }),
  );
}
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      const defaultIds =
        "bitcoin,ethereum,ethereum-classic,litecoin,ravencoin,monero,kaspa,iron-fish,zephyr-protocol,clore-ai,dynex,conflux,ergo";
      const idsParam = req.query.ids || defaultIds;
      const requestedIds = idsParam.split(",").map((s) => s.trim());

      try {
        // 1. Try to get prices from the database (latest snapshot)
        let dbPrices = await getCoinPricesFromDb(requestedIds, 200);

        // 2. If no data in DB or it's stale, trigger a fresh fetch (optional)
        if (!dbPrices || Object.keys(dbPrices).length === 0) {
          console.log("[CoinGecko] No DB prices found, fetching fresh...");
          await fetchAndSaveCoinPrices(true);
          dbPrices = await getCoinPricesFromDb(requestedIds, 200);
        }

        // 3. Format response to match the expected structure (coinId -> { usd, btc })
        const result = {};
        for (const id of requestedIds) {
          if (dbPrices[id]) {
            result[id] = {
              usd: dbPrices[id].usd || 0,
              btc: dbPrices[id].btc || 0,
            };
          } else {
            // Fallback to hardcoded rates if missing
            const fallback = FALLBACK_BTC_RATES[id];
            if (fallback !== undefined) {
              result[id] = { usd: 0, btc: fallback };
            } else {
              result[id] = { usd: 0, btc: 0 };
            }
          }
        }

        // Always ensure bitcoin has btc:1
        if (result["bitcoin"]) result["bitcoin"].btc = 1;
        ...
        console.error("[CoinGecko] Failed to get prices:", err.message);
        // Return fallback rates on error
        const fallback = buildFallbackPrices(idsParam);
        return res.json({
          success: true,
          data: fallback,
          fallback: true,
