...
async function fetchNiceHashPrice(algorithm, baseUrl = `http://localhost:${process.env.PORT || 3000}`) {
  ...
  try {
    ...
    const url = `${baseUrl}/api/v2/hashpower/order/price?algorithm=${encodeURIComponent(nhAlgo)}&client=ALL`;
...
export async function getHeroMinersProfitEstimates(force = false, btcPriceOverride = null) {
  ...
  const coinSymbols = [...new Set(coinStats.map(s => getCoinGeckoId(s.coin) || '').filter(Boolean))];
  let coinPrices = {};
  try {
  ...
  for (const stat of coinStats) {
    ...
    const coinPriceId = getCoinGeckoId(coinName);
    const coinPrice = coinPrices[coinPriceId]?.usd || 0;
