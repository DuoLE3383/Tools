...
import { normalizeAlgo, getNiceHashUnit, HASHRATE_SUFFIXES } from "../../src/core/mapping.js";
import { getHeroMinersProfitEstimates } from "../miners/herominers-profit-estimates.js";

function getProfitGuardPrice(estimate, nhUnit, minimumProfitPercent = 0) {
  const poolHashrate = Number(estimate?.poolHashrate || 0);
  const poolBtcPerDay = Number(estimate?.poolBtcPerDay || 0);
  const nhUnitH = HASHRATE_SUFFIXES[String(nhUnit || "H").toUpperCase()] || 1;
  const poolUnitH = HASHRATE_SUFFIXES[String(estimate?.poolHashrateUnit || "H").toUpperCase()] || 1;
  const poolHashrateNhUnits = (poolHashrate * poolUnitH) / nhUnitH;
  if (poolBtcPerDay <= 0 || poolHashrateNhUnits <= 0) return null;

  const marginMultiplier = 1 + Math.max(0, Number(minimumProfitPercent) || 0) / 100;
  return poolBtcPerDay / poolHashrateNhUnits / marginMultiplier;
}
...
export async function registerNiceHashUpdateRatesRoute(app) {
  app.post('/api/v2/hashpower/orders/update-prices', async (req, res) => {
    try {
      ...
      const profitGuard = req.body?.profitGuard || null;
      const profitEstimates = profitGuard?.enabled
        ? await getHeroMinersProfitEstimates(false, profitGuard.btcPrice)
        : null;
      const requestedCoin = String(profitGuard?.coin || '').trim().toUpperCase();
      const minimumProfitPercent = Math.max(0, Number(profitGuard?.minimumProfitPercent) || 0);
      const dryRun = profitGuard?.enabled && profitGuard?.dryRun !== false;
      const clientParam = String(req.body?.client || req.query?.client || 'ALL').toUpperCase();
      ...
      for (const acct of nhAccounts) {
        ...
        for (const order of activeOrders) {
          ...
          // Apply premium
          let newPrice = marketInfo.price * (1 + premium);
          let profitCap = null;
          let matchedEstimate = null;
          if (profitGuard?.enabled) {
            matchedEstimate = (profitEstimates?.estimates || []).find((estimate) => {
              const sameAlgo = normalizeAlgoForNiceHash(estimate.algorithm) === normalizeAlgoForNiceHash(algo);
              const sameCoin = !requestedCoin || String(estimate.coin || '').toUpperCase() === requestedCoin;
              return sameAlgo && sameCoin;
            });
            profitCap = getProfitGuardPrice(matchedEstimate, getNiceHashUnit(algo), minimumProfitPercent);
            if (profitCap) newPrice = Math.min(newPrice, profitCap);
          }

          const newLimit = profitCap && newPrice > 0
            ? Math.min(currentLimit, (matchedEstimate.poolBtcPerDay / newPrice) *
              ((HASHRATE_SUFFIXES[String(getNiceHashUnit(algo)).toUpperCase()] || 1) /
                (HASHRATE_SUFFIXES[String(matchedEstimate.poolHashrateUnit || 'H').toUpperCase()] || 1)))
            : currentLimit;

          if (profitGuard?.enabled && !matchedEstimate) {
            clientResults.push({ orderId, algorithm: algo, status: 'skipped', reason: requestedCoin ? `No HeroMiners estimate for ${requestedCoin}` : 'No HeroMiners estimate for algorithm' });
            continue;
          }
          if (profitGuard?.enabled && !profitCap) {
            clientResults.push({ orderId, algorithm: algo, status: 'skipped', reason: 'HeroMiners estimate has no usable pool hashrate' });
            continue;
          }
          ...
          try {
            ...
            clientResults.push({
              ...
              limit: newLimit,
              profitCap,
              profitGuard: Boolean(profitGuard?.enabled),
