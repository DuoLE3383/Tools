...
export function isRealRental(rental, info, now = Date.now()) {
    if (!rental || !info) {
        console.debug(`[monitor:isRealRental] Skipped due to missing rental or info object.`);
        return false;
    }

    const rentalId = rental.id || rental.rentalid || rental.rental_id;
    if (!rentalId) {
        console.debug(`[monitor:isRealRental] Skipped due to missing rental ID.`);
        return false;
    }
    ...
    if (!rawStart) {
        if (parseFloat(rental.price) > 0) {
            console.debug(`[monitor:isRealRental] Rental ${rentalId} is real (has price).`);
            return true;
        }
        console.debug(`[monitor:isRealRental] Rental ${rentalId} is NOT real (no activity and no start time).`);
        return false;
    }
    ...
    if (ageMs > 0 && ageMs < 60 * 60 * 1000) {
        console.debug(`[monitor:isRealRental] Rental ${rentalId} is considered real (in 1-hour grace period).`);
        return true;
    }
    ...
    if (advertisedHash > 0 || price > 0) {
        console.debug(`[monitor:isRealRental] Rental ${rentalId} is real (has advertised hashrate or price).`);
        return true;
    }

    console.debug(`[monitor:isRealRental] Rental ${rentalId} is a ghost (age: ${Math.round(ageMs / 1000)}s, no activity).`);
    return false;
}
...
export async function processRental(rental, acct, now, forceNotify, notifiedRentalIdsThisRun, notifications, liveRig = null) {
    ...
    if (!isValidRental) {
        console.debug(`[monitor:process] Skipped invalid or ghost rental: ${rental.id || 'N/A'}`);
